...
- **Three view modes** — Standard (strikethrough), Clean (cuts hidden, final script only), or Diff (side-by-side comparison with original); in Diff view the right column always shows the unmodified original — no splits, no insertions, no edits applied; the Script nav link is a dropdown on every page so you can switch mode and jump to the script in one click from anywhere in the app; if a scene has been subdivided into A/B/C parts, amber "Part B / Part C" divider lines appear at the split boundaries in all three view modes (view-only — editing is done from the Scenes & Pauses dashboard)
...
### Scene Dashboard
- **Scenes & Pauses** — see all scenes in order with their cut counts; drag to reorder scenes; insert named pauses (intermission, breaks) between scenes with a duration that adds to total running time; **split any scene into A/B/C sub-parts** via the ✂ Split button — enter a line number, preview the 2 lines before and after the split point, and confirm; sub-parts appear as indented A/B/C rows with their own line/word/time counts and × remove buttons; intermissions can be inserted between sub-parts independently
- **Matrix** — character × scene grid showing who speaks (or is on stage) in each scene; columns group by actor; when a scene is subdivided, it expands to A/B/C sub-columns with amber labels; click a column to filter; row and column totals; toggle between Table and Chart views
- **Chart** — Bar mode shows a sorted horizontal bar chart of totals per character; **Presence mode** shows a Tableau-style two-panel visualization: a per-character swimlane across the full play (speech bars positioned by Folger through-line number, act headers at the top) plus per-scene strips below proportionally sized by scene length; click a character name or scene label to cross-filter both panels
- **Rehearsal planner** — per-actor scene breakdown and suggested rehearsal blocks; scenes are split into sub-scenes at major cast entrances then grouped by shared cast using complete-linkage hierarchical clustering (Jaccard similarity); when director-defined A/B/C subdivisions exist, a **Scenes/Sub-scenes toggle** lets you choose whether the algorithm treats each sub-part as an independent unit; toggle between by-character and by-actor grouping; full-company scenes (≥10 min with nearly the entire cast) are always isolated; actor chips show which characters they play in that block; block duration limits are configurable in Settings; filter the By Actor list by actor or character name; collapse any actor row to hide their scene breakdown
...
- **Import cut from Word** *(experimental)* — directors who work "old fashioned" by highlighting text in Word can import that `.docx` directly as a new cut; the tool reads highlight annotations only (ignores tracked changes, insertions, stage directions, speaker labels, and any text that doesn't match the Folger edition); the document must have clear Act/Scene headings; available in ⚙ Settings → "Import cut from pre-existing Word doc"
