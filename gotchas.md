...
- **Group 30**: `cut.speechReassignments` is typed `Record<string, string[]>` (one-to-many), not `Record<string, string>`. The plan spec described it loosely as `characterId → characterId`; always check `types/project.ts` before assuming the value type.
- **Group 30**: local dev password in memory was stale — the hash in `.env` changed without updating the memory entry. Avoid storing hashed-password-dependent credentials in memory; use `AUTH_DISABLED=true` in `.env` for local dev instead.
