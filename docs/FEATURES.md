...
| **Print from nav** | Printer icon in the script editor nav bar (desktop + hamburger menu) calls `window.print()` directly; hidden during printing |
| **Line Buddy HTML (per-actor)** | "Export Line Buddy" button next to Print/Save PDF downloads the current actor's HTML drill file directly without a ZIP |
| **Line Buddy HTML (all actors)** | "All Line Buddies (ZIP)" in the batch section downloads one HTML file per actor in a single ZIP |
| **Line Buddy — scene-based layout** | Scene-by-scene scrolling layout; all content visible as a column; `lines` items start hidden and are revealed one at a time with Space/→; sticky header shows act · scene with a jump `<select>` and prev/next buttons; progress shows "N / M lines" within the scene; `[`/`]` prev/next scene; `g` focuses scene select; song speeches violet italic; song SDs show ♪ prefix, dance SDs show ⊛ prefix |
