...
Project
  ...
  cuts[]
    ...
    sdTextEdits?    — Record<sdId, string>  (cosmetic SD prose rewrites)
    sceneSubdivisions? — Record<sceneId, SubdivisionSplit[]>  (A/B/C sub-part splits)
    actNotes?       — Record<actId, string>  (production notes per act)
    sceneNotes?     — Record<sceneId, string>  (production notes per scene)
  actors[]
    name, color
  assignments[]
    characterId → actorId
  settings?
    ...
    rehearsalMinBlockMinutes
    rehearsalMaxBlockMinutes
    minActorStageTimeMinutes
```
